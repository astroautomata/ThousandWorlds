...
def run(
    ...
    **overrides,
    ...
    args = argparse.Namespace(
        ...
        convdec_seed_channels=128,
        convdec_convs_per_stage=1,
        convdec_batch_size=32,
        convdec_num_steps=3000,
        convdec_lr=3.0e-4,
        convdec_weight_decay=1.0e-4,
        convdec_hard_stop_step=None,
        sfno_embed_dim=128,
        sfno_num_layers=4,
        sfno_batch_size=32,
        sfno_num_steps=3000,
...
def _prepare_net_target(args: argparse.Namespace, data, Y: np.ndarray):
    if not bool(getattr(args, "target_detrend", False)):
        return Y, None, None
    field_names = data.spectral_bundle.raw_field_names
    transform = fit_net_target_detrender(
        data.X_train_std,
        data.s_train,
        tw.normalise_spectral(data.spectral_bundle.Y_train, field_names, data.stats),
        field_mask=data.spectral_bundle.field_mask_train,
        field_names=field_names,
        stats=data.stats,
        sh_mask=data.sh_mask,
        inverse_sht=data.inverse_sht,
    )
    residual = transform.remove_grid(Y, data.X_train_std, data.s_train)
    restored = transform.apply_grid(residual, data.X_train_std, data.s_train)
    valid = data.grid_bundle.field_mask_train[:, :, None, None] & np.isfinite(Y)
    round_trip_max_abs = float(np.max(np.abs(restored[valid] - Y[valid])))
    round_trip_tolerance = float(
        8.0 * np.finfo(np.float32).eps * max(1.0, float(np.max(np.abs(Y[valid]))))
    )
    zero = transform.apply_grid(
        transform.zero_residual_grid(len(Y)), data.X_train_std, data.s_train
    )
    zero_residual_max_abs = float(
        np.max(np.abs(zero - transform.bare_trend_grid(data.X_train_std, data.s_train)))
    )
    if round_trip_max_abs > round_trip_tolerance or zero_residual_max_abs > round_trip_tolerance:
        raise FloatingPointError("target detrend round-trip gate failed")
    return residual, transform, {
        "enabled": True,
        "design": dict(transform.trend.design_cfg),
        "lambda": float(transform.trend.lambda_reg),
        "round_trip_max_abs": round_trip_max_abs,
...
def _run_knn(args: argparse.Namespace) -> dict:
    ...
    if args.best_k is not None and args.best_gcm_penalty is not None:
        ...
        for train_idx, val_idx in _kfold_indices(
            len(data.X_train_std), n_folds=args.n_folds, seed=args.seed
        ):
            fold = prepare_tw_fold(data, train_idx)
            Y_fold = average_space_grid(
                fold.grid_bundle.Y_train,
                fold.grid_bundle.raw_field_names,
                fold.stats,
                X=fold.grid_bundle.X_train,
            )
            folds.append(
                (
                    train_idx,
                    ...
                    field_rmse_scale_grid(
                        Y_fold[train_idx],
                        fold.grid_bundle.field_mask_train[train_idx],
                    ),
                )
            )
        ...
        for penalty in penalty_grid:
            ...
            for k in k_grid:
                fold_losses = []
                for train_idx, val_idx, fold, Y_fold, field_scale in folds:
                    X_fold = _append_gcm_block(
                        fold.X_train_std,
                        fold.s_train,
                        len(fold.gcm_labels),
                        penalty,
                    ...
                    model.fit(
                        ...
                        Y_fold[train_idx],
                        k_candidates=[k],
                        field_mask=fold.grid_bundle.field_mask_train[train_idx],
                    )
                    fold_losses.append(
                        equal_group_normalized_rmse_grid(
                            enforce_equatorial_symmetry_grid(
                                model.predict(X_fold[val_idx], k),
                                fold.grid_bundle.raw_field_names,
                            ),
                            Y_fold[val_idx],
                            fold.grid_bundle.raw_field_names,
                            field_scale,
                            fold.grid_bundle.field_mask_train[val_idx],
                        )
                    )
    ...
    Y_train_avg = average_space_grid(
        data.grid_bundle.Y_train,
        data.grid_bundle.raw_field_names,
        data.stats,
        X=data.grid_bundle.X_train,
    )
...
def _run_pca_ridge(args: argparse.Namespace) -> dict:
    ...
    if hparams["best_latent_dim"] is not None and hparams["best_lambda_reg"] is not None:
        ...
        folds = []
        for train_idx, val_idx in _eligible_kfold_indices(
            data.grid_bundle.meta_train, n_folds=int(hparams["n_folds"]), seed=args.seed
        ):
            fold_data = prepare_tw_fold(data, train_idx)
            Y_fold = np.transpose(
                tw.normalise_spectral(
                    fold_data.spectral_bundle.Y_train,
                    fold_data.spectral_bundle.raw_field_names,
                    fold_data.stats,
                ),
                (0, 2, 1),
            )
            Y_fold_avg = average_space_grid(
                fold_data.grid_bundle.Y_train,
                fold_data.grid_bundle.raw_field_names,
                fold_data.stats,
                X=fold_data.grid_bundle.X_train,
            )
            folds.append(
                (
                    train_idx,
                    val_idx,
                    field_rmse_scale_grid(Y_fold_avg[train_idx], fold_data.grid_bundle.field_mask_train[train_idx]),
                    fold_data,
                    Y_fold,
                    Y_fold_avg,
                )
        ...
        for i, latent_dim in enumerate(latent_grid):
            fold_models = []
            for train_idx, val_idx, field_scale, fold_data, Y_fold, Y_fold_avg in folds:
                model = PCARidge(
                ...
                model.fit(
                    torch.from_numpy(fold_data.X_train_std[train_idx]),
                    torch.from_numpy(fold_data.s_train[train_idx]),
                    torch.from_numpy(Y_fold[train_idx]),
                    field_mask=torch.from_numpy(fold_data.spectral_bundle.field_mask_train[train_idx]),
                    sh_mask=torch.from_numpy(fold_data.sh_mask),
                    linear_trend_cfg=PCA_RIDGE_LINEAR_TREND_CFG,
                    ppca_iters=int(hparams["ppca_iters"]),
                    seed=args.seed,
                    n_sim_types=len(fold_data.gcm_labels),
                )
                H_train = build_design_matrix(
                    torch.as_tensor(fold_data.X_train_std[train_idx], device=model.device, dtype=model.dtype),
                    torch.as_tensor(fold_data.s_train[train_idx], device=model.device, dtype=torch.long),
                    n_sim_types=model.n_sim_types_,
                    design_cfg=PCA_RIDGE_DESIGN_CFG,
                )
                fold_models.append((model, H_train, val_idx, field_scale, fold_data, Y_fold_avg))
            for j, lambda_reg in enumerate(lambda_grid):
                fold_losses = []
                for model, H_train, val_idx, field_scale, fold_data, Y_fold_avg in fold_models:
                    ...
                    pred_coeffs = model.predict(
                        torch.from_numpy(fold_data.X_train_std[val_idx]),
                        torch.from_numpy(fold_data.s_train[val_idx].copy()),
                    ).detach().cpu().numpy().transpose(0, 2, 1)
                    pred_avg = decode_spectral_predictions(
                        pred_coeffs,
                        fold_data.spectral_bundle.raw_field_names,
                        fold_data.stats,
                        sh_mask=fold_data.sh_mask,
                        inverse_sht=fold_data.inverse_sht,
                    )
                    fold_losses.append(
                        equal_group_normalized_rmse_grid(
                            pred_avg,
                            ...
                            fold_data.grid_bundle.raw_field_names,
                            field_scale,
                            fold_data.grid_bundle.field_mask_train[val_idx],
                        )
                    )
                mean_loss = float(np.mean(fold_losses))
    ...
    Y_train = np.transpose(
        tw.normalise_spectral(data.spectral_bundle.Y_train, data.spectral_bundle.raw_field_names, data.stats),
        (0, 2, 1),
    )
    model = PCARidge(
...
def _run_pca_gbt(args: argparse.Namespace) -> dict:
    ...
    def _fit_full(model: PCAGBT, idx, fit_data, fit_Y) -> PCAGBT:
        model.fit(
            torch.from_numpy(fit_data.X_train_std[idx]),
            torch.from_numpy(fit_data.s_train[idx].copy()),
            torch.from_numpy(fit_Y[idx]),
            field_mask=torch.from_numpy(fit_data.spectral_bundle.field_mask_train[idx]),
            sh_mask=torch.from_numpy(fit_data.sh_mask),
            linear_trend_cfg=PCA_GBT_LINEAR_TREND_CFG,
            field_names=fit_data.spectral_bundle.raw_field_names,
            ppca_iters=int(args.ppca_iters),
            seed=args.seed,
            n_sim_types=len(fit_data.gcm_labels),
        )
        return model
    ...
    if have_best:
        ...
        for train_idx, val_idx in _eligible_kfold_indices(data.grid_bundle.meta_train, n_folds=int(hp["n_folds"]), seed=args.seed):
            ...
            model = _fit_full(_make_model(lr_grid[0], mln_grid[0]), train_idx, fold_data, Y_fold)
            Xin = model._build_inputs_np(
                torch.as_tensor(fold_data.X_train_std[train_idx], device=model.device, dtype=model.dtype),
                torch.as_tensor(fold_data.s_train[train_idx], device=model.device, dtype=torch.long),
            )
            Z_target = model.ppca_.Z.detach().cpu().numpy()
            field_scale = field_rmse_scale_grid(
                Y_fold_avg[train_idx], fold_data.grid_bundle.field_mask_train[train_idx]
            )
            folds.append((model, Xin, Z_target, val_idx, field_scale, fold_data, Y_fold_avg))
        for i, learning_rate in enumerate(lr_grid):
            for j, max_leaf_nodes in enumerate(mln_grid):
                fold_losses = []
                for model, Xin, Z_target, val_idx, field_scale, fold_data, Y_fold_avg in folds:
    ...
    model = _fit_full(_make_model(best["learning_rate"], best["max_leaf_nodes"]), slice(None), data, Y_train)
    pred_coeffs = model.predict(torch.from_numpy(data.X_test_std), torch.from_numpy(data.s_test.copy())).detach().cpu().numpy().transpose(0, 2, 1)
    pred_avg = decode_spectral_predictions(pred_coeffs, data.spectral_bundle.raw_field_names, data.stats, sh_mask=data.sh_mask, inverse_sht=data.inverse_sht)
    predictions = inverse_average_space_grid(pred_avg, data.spectral_bundle.raw_field_names, data.stats, X=data.grid_bundle.X_test)[None]
    return {
...
def _run_coord_mlp(args: argparse.Namespace) -> dict:
    ...
    Y_train_avg, target_detrender, target_detrend = _prepare_net_target(args, data, Y_train_avg)
    model = CoordMLP(
    ...
    pred_avg = model.predict(data.X_test_std, data.s_test).detach().cpu().numpy()
    if target_detrender is not None:
        pred_avg = target_detrender.apply_grid(pred_avg, data.X_test_std, data.s_test)
    ...
    return {"data": data, "predictions": predictions, "meta": {"method": "coord_mlp", "fit": model.fit_stats_, "equatorial_symmetry": True, "target_detrend": target_detrend}}
...
def _run_coord_deeponet(args: argparse.Namespace) -> dict:
    ...
    predictions = inverse_average_space_grid(pred_avg, data.grid_bundle.raw_field_names, data.stats, X=data.grid_bundle.X_test)[None]
    return {"data": data, "predictions": predictions, "meta": {"method": "coord_deeponet", "fit": model.fit_stats_, "equatorial_symmetry": True, "target_detrend": target_detrend}}


def _run_conv_decoder(args: argparse.Namespace) -> dict:
    from thousandworlds.models.conv_decoder import ConvDecoder

    torch = _torch()
    data = prepare_tw_data(args.subset, data_dir=args.data_dir)
    hparams = _convdec_hparams(args)
    Y_train = average_space_grid(data.grid_bundle.Y_train, data.grid_bundle.raw_field_names, data.stats, X=data.grid_bundle.X_train)
    Y_train, target_detrender, target_detrend = _prepare_net_target(args, data, Y_train)
    model = ConvDecoder(
        seed_channels=int(hparams["convdec_seed_channels"]),
        convs_per_stage=int(hparams["convdec_convs_per_stage"]),
        batch_size=int(hparams["convdec_batch_size"]),
        dtype=getattr(torch, args.dtype),
        device=args.device,
    )
    model.fit(
        ...
        field_names=data.grid_bundle.raw_field_names,
        n_sim_types=len(data.gcm_labels),
        num_steps=int(hparams["convdec_num_steps"]),
        lr=float(hparams["convdec_lr"]),
        weight_decay=float(hparams["convdec_weight_decay"]),
        seed=args.seed,
        hard_stop_step=hparams["convdec_hard_stop_step"],
    )
    pred = model.predict(data.X_test_std, data.s_test).detach().cpu().numpy()
    if target_detrender is not None:
        pred = target_detrender.apply_grid(pred, data.X_test_std, data.s_test)
    pred = enforce_equatorial_symmetry_grid(
        pred,
        data.grid_bundle.raw_field_names,
    )
    predictions = inverse_average_space_grid(pred, data.grid_bundle.raw_field_names, data.stats, X=data.grid_bundle.X_test)[None]
    return {"data": data, "predictions": predictions, "meta": {"method": "conv_decoder", "fit": model.fit_stats_, "equatorial_symmetry": True, "target_detrend": target_detrend}}


def _run_sfno(args: argparse.Namespace) -> dict:
    from thousandworlds.models.sfno import SFNO

    torch = _torch()
    data = prepare_tw_data(args.subset, data_dir=args.data_dir)
    hparams = _sfno_hparams(args)
    Y_train = average_space_grid(data.grid_bundle.Y_train, data.grid_bundle.raw_field_names, data.stats, X=data.grid_bundle.X_train)
    Y_train, target_detrender, target_detrend = _prepare_net_target(args, data, Y_train)
    model = SFNO(
        embed_dim=int(hparams["sfno_embed_dim"]),
        num_layers=int(hparams["sfno_num_layers"]),
        batch_size=int(hparams["sfno_batch_size"]),
        dtype=getattr(torch, args.dtype),
        device=args.device,
    )
    model.fit(
        ...
        field_names=data.grid_bundle.raw_field_names,
        n_sim_types=len(data.gcm_labels),
        num_steps=int(hparams["sfno_num_steps"]),
        lr=float(hparams["sfno_lr"]),
        weight_decay=float(hparams["sfno_weight_decay"]),
        seed=args.seed,
        hard_stop_step=hparams["sfno_hard_stop_step"],
    )
    pred = model.predict(data.X_test_std, data.s_test).detach().cpu().numpy()
    if target_detrender is not None:
        pred = target_detrender.apply_grid(pred, data.X_test_std, data.s_test)
    ...
    predictions = inverse_average_space_grid(pred, data.grid_bundle.raw_field_names, data.stats, X=data.grid_bundle.X_test)[None]
    return {"data": data, "predictions": predictions, "meta": {"method": "sfno", "fit": model.fit_stats_, "equatorial_symmetry": True, "target_detrend": target_detrend}}
...
def _run_ppca_icm(args: argparse.Namespace) -> dict:
    ...
    torch = _torch()
    data = prepare_tw_data(args.subset, data_dir=args.data_dir)
    hparams = _ppca_icm_hparams(args, getattr(args, "_config", None))
    Y_train = np.transpose(tw.normalise_spectral(data.spectral_bundle.Y_train, data.spectral_bundle.raw_field_names, data.stats), (0, 2, 1))
    model = PPCAICM(
        latent_dim=int(hparams["latent_dim"]),
...
def _run_method(args: argparse.Namespace) -> dict:
    return {
        ...
        "conv_decoder": _run_conv_decoder,
        "sfno": _run_sfno,
...
def main() -> None:
    ...
    parser.add_argument("method", nargs="?", choices=["train_mean", "knn", "pca_ridge", "pca_mlp", "pca_gbt", "ppca_icm", "gplfr", "coord_mlp", "coord_deeponet", "conv_decoder", "sfno"])
    ...
    parser.add_argument("--target-detrend", action="store_true", help="Experimental: train field-space nets on GPLFR-style per-GCM trend residuals")
    ...
    parser.add_argument("--pca-mlp-depth", dest="depth", type=int, default=2, help="PCA-MLP: Number of hidden layers")
    ...
    # ---- Conv-Decoder arguments ----
    parser.add_argument("--convdec-seed-channels", type=int, default=128, help="Conv-Decoder: Seed-grid channels")
    parser.add_argument("--convdec-convs-per-stage", type=int, default=1, help="Conv-Decoder: Convolutions per resize stage")
    parser.add_argument("--convdec-batch-size", type=int, default=32, help="Conv-Decoder: Example batch size")
    parser.add_argument("--convdec-num-steps", type=int, default=3000, help="Conv-Decoder: Number of training steps")
    parser.add_argument("--convdec-lr", type=float, default=3.0e-4, help="Conv-Decoder: Learning rate")
    parser.add_argument("--convdec-weight-decay", type=float, default=1.0e-4, help="Conv-Decoder: Weight decay")
    parser.add_argument("--convdec-hard-stop-step", type=int, default=None, help="Conv-Decoder: Optional hard stop step")

    # ---- SFNO arguments ----
    parser.add_argument("--sfno-embed-dim", type=int, default=128, help="SFNO: Embedding dimension")
    parser.add_argument("--sfno-num-layers", type=int, default=4, help="SFNO: Number of operator blocks")
    parser.add_argument("--sfno-batch-size", type=int, default=32, help="SFNO: Example batch size")
    parser.add_argument("--sfno-num-steps", type=int, default=3000, help="SFNO: Number of training steps")
    parser.add_argument("--sfno-lr", type=float, default=3.0e-4, help="SFNO: Learning rate")
    parser.add_argument("--sfno-weight-decay", type=float, default=1.0e-4, help="SFNO: Weight decay")
    parser.add_argument("--sfno-hard-stop-step", type=int, default=None, help="SFNO: Optional hard stop step")
    ...
    parser.add_argument("--gplfr-kernel", choices=["rbf", "matern32", "matern52"], default="matern52", help=argparse.SUPPRESS)
    parser.add_argument("--gplfr-n-samples", type=int, default=64, help=argparse.SUPPRESS)
    ...
    if tw.supports_protocol(args.subset, "shared_planets"):
        ...
        shared_path = save_submission(out_dir / "predictions_shared_planets.npz", shared_predictions, simulation_id=shared_ids, field_names=data.grid_bundle.field_names)
        shared_point_path = save_submission(out_dir / "predictions_mean_shared_planets.npz", shared_point_predictions, simulation_id=shared_ids, field_names=data.grid_bundle.field_names)
