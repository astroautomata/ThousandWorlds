...
TW_ROOT = Path(__file__).resolve().parents[2]
...
def prepare_tw_fold(data: PreparedTWData, train_idx: np.ndarray) -> PreparedTWData:
    """Return a view transformed with statistics fitted only on one fold's training rows."""
    train_idx = np.asarray(train_idx, dtype=np.int64)
    stats = data.stats
    normalize_mean, normalize_std = {}, {}
    for i, name in enumerate(stats.input_names):
        strategy_name, kwargs = tw.preprocessing._strategy_params(name, stats)
        key = tw.preprocessing._stat_key(name, stats)
        values = tw.preprocessing._preprocess_array(data.grid_bundle.X_train[train_idx, i], strategy_name, kwargs)
        normalize_mean[key] = np.asarray([values.mean(dtype=np.float64)], dtype=np.float32)
        normalize_std[key] = np.asarray([values.std(dtype=np.float64)], dtype=np.float32)

    spectral = {}
    for j, name in enumerate(data.spectral_bundle.raw_field_names):
        template = stats.spectral[name]
        allowed = data.spectral_bundle.Y_train[train_idx][
            data.spectral_bundle.field_mask_train[train_idx, j], j
        ][:, template.mask]
        if not len(allowed):
            raise ValueError(f"Fold training rows contain no observations for {name}.")
        mean = allowed.mean(axis=0, dtype=np.float64).astype(np.float32)
        energy_mean = float(np.mean(np.sum(np.square(allowed.astype(np.float64)), axis=1)))
        mean64 = mean.astype(np.float64)
        sigma = float(np.float32(max(np.sqrt(max(energy_mean - float(np.dot(mean64, mean64)), 0.0)), 1.0e-8)))
        spectral[name] = replace(template, mean=mean, sigma=sigma, count=len(allowed), energy_mean=energy_mean)
    ...
    fold_stats = replace(
        ...
        spectral_meta={
            **stats.spectral_meta,
            "fields": {
                name: {**stats.spectral_meta["fields"][name], "count": record.count, "sigma": record.sigma}
                for name, record in spectral.items()
            },
        },
    )
    return replace(
        data,
        stats=fold_stats,
        X_train_std=tw.transform_inputs(data.grid_bundle.X_train, fold_stats),
        X_test_std=tw.transform_inputs(data.grid_bundle.X_test, fold_stats),
    )
...
def save_json(path: str | Path, payload: dict) -> Path:
    ...
    tmp.write_text(json.dumps(_to_jsonable(payload), indent=2) + "\n", encoding="utf-8")
    tmp.replace(path)
    return path
