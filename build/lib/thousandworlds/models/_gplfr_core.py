...
class GPLFRCore:
    ...
    def _build_svi_map(
        ...
        *,
        ...
        optimizer = pyro.optim.ExponentialLR({"optimizer": torch.optim.Adam, "optim_args": optim_args, "gamma": float(lr_gamma)})
    ...
    def fit(
        ...
        latent_parameterization: Literal["direct_z", "whitened"] = "direct_z",
    ):
        """
        Fit the released MAP GPLFR recipe.
        """
        ...
        if latent_parameterization not in {"direct_z", "whitened"}:
            raise ValueError("latent_parameterization must be 'direct_z' or 'whitened'.")
        self._latent_param_mode = latent_parameterization
        ...
        for step in range(total_steps):
            ...
            if checkpoint_callback is not None:
                checkpoint_callback(self, guide, step)
            if diagnostics_callback is not None:
                diagnostics_callback(self, guide, svi, step, loss, Y, X, s)
