...
from .models._target_detrend import TARGET_DETREND_CFG
...
PCA_MLP_PRESETS = {
    "multi-partial": {"latent_dim": 50, "hidden_width": 1024, "depth": 2, "num_steps": 4000, "lr": 3.0e-4, "weight_decay": 1.0e-4, "hard_stop_step": 860},
    "multi-complete": {"latent_dim": 50, "hidden_width": 1024, "depth": 2, "num_steps": 4000, "lr": 3.0e-4, "weight_decay": 1.0e-4, "hard_stop_step": 630},
    "single-complete": {"latent_dim": 150, "hidden_width": 512, "depth": 2, "num_steps": 4000, "lr": 3.0e-4, "weight_decay": 1.0e-4, "hard_stop_step": 330},
}
...
CONVDEC_PRESETS: dict[str, dict] = {
    "multi-partial": {"seed_channels": 2048, "convs_per_stage": 1, "batch_size": 32, "num_steps": 3000, "lr": 3.0e-4, "weight_decay": 0.3, "hard_stop_step": 2900},
    "multi-complete": {"seed_channels": 2048, "convs_per_stage": 1, "batch_size": 32, "num_steps": 3000, "lr": 3.0e-4, "weight_decay": 0.3, "hard_stop_step": 2700},
    "single-complete": {"seed_channels": 1024, "convs_per_stage": 2, "batch_size": 32, "num_steps": 3000, "lr": 3.0e-4, "weight_decay": 0.3, "hard_stop_step": 2700},
}
...
SFNO_PRESETS: dict[str, dict] = {
    "multi-partial": {"embed_dim": 768, "num_layers": 2, "batch_size": 32, "num_steps": 3000, "lr": 3.0e-4, "weight_decay": 0.3, "hard_stop_step": 3000},
    "multi-complete": {"embed_dim": 768, "num_layers": 2, "batch_size": 32, "num_steps": 3000, "lr": 3.0e-4, "weight_decay": 0.3, "hard_stop_step": 2900},
    "single-complete": {"embed_dim": 768, "num_layers": 2, "batch_size": 32, "num_steps": 3000, "lr": 3.0e-4, "weight_decay": 0.3, "hard_stop_step": 2600},
}
...
def _gplfr_hparams(args: argparse.Namespace, cfg: dict | None = None) -> dict[str, int | float | str | None]:
    if cfg is not None and "gplfr" in cfg:
        ...
        values.update({k: cfg["gplfr"][k] for k in ("kernel", "latent_dim", "num_training_steps", "inverse_temperature", "latent_nugget", "n_samples", "variable_weights", "output_coregionalization") if k in cfg["gplfr"]})
        values.update((cfg["gplfr"].get("optimizer") or {}))
        return values
    values = dict(GPLFR_PRESETS.get(args.subset, GPLFR_ARG_DEFAULTS))
    explicit = set(getattr(args, "_explicit_args", set()))
    for key in ("latent_dim", "gplfr_kernel", "gplfr_num_training_steps", "gplfr_inverse_temperature", "gplfr_latent_nugget", "gplfr_n_samples", "gplfr_variable_weights", "gplfr_output_coregionalization"):
        if key in explicit:
...
def _prefixed_hparams(
    ...
    *,
    ...
    if cfg is not None and method in cfg:
        values = dict(defaults)
        values.update(
            {
                str(k) if str(k).startswith(prefix) else f"{prefix}{k}": v
                for k, v in cfg[method].items()
                if k != "preset"
            }
        )
        return values
    values = {key: getattr(args, key) for key in defaults}
    return _prefixed_hparams(args, {method: presets[args.subset]}, method=method, prefix=prefix, defaults=defaults, presets=presets) if values == defaults and args.subset in presets else values


def _convdec_hparams(args: argparse.Namespace, cfg: dict | None = None) -> dict[str, int | float | str | None]:
    return _prefixed_hparams(args, cfg, method="conv_decoder", prefix="convdec_", defaults=CONVDEC_ARG_DEFAULTS, presets=CONVDEC_PRESETS)


def _sfno_hparams(args: argparse.Namespace, cfg: dict | None = None) -> dict[str, int | float | str | None]:
    return _prefixed_hparams(args, cfg, method="sfno", prefix="sfno_", defaults=SFNO_ARG_DEFAULTS, presets=SFNO_PRESETS)
...
def _merge_config_args(args: argparse.Namespace, parser: argparse.ArgumentParser) -> tuple[argparse.Namespace, dict | None]:
    ...
    if isinstance(method_cfg, dict) and args.method not in {"coord_mlp", "coord_deeponet", "conv_decoder", "sfno", "pca_ridge", "pca_gbt"}:
        for key, default in defaults.items():
    ...
    if args.method == "coord_deeponet" and isinstance(method_cfg, dict):
        for key in COORD_DEEPONET_ARG_DEFAULTS:
            ...
            if getattr(args, key) == defaults[key] and bare in method_cfg:
                setattr(args, key, method_cfg[bare])
    for method, prefix, arg_defaults in (
        ("conv_decoder", "convdec_", CONVDEC_ARG_DEFAULTS),
        ("sfno", "sfno_", SFNO_ARG_DEFAULTS),
    ):
        if args.method == method and isinstance(method_cfg, dict):
            for key in arg_defaults:
                bare = key.removeprefix(prefix)
                if key not in getattr(args, "_explicit_args", set()) and getattr(args, key) == defaults[key] and bare in method_cfg:
                    setattr(args, key, method_cfg[bare])
...
def _resolved_config(args: argparse.Namespace, *, out_dir: Path, data_dir: Path, meta: dict | None = None) -> dict:
    ...
    if args.method == "knn":
        ...
        cfg["conv_decoder"] = {
            ...
            "hard_stop_step": None if hparams["convdec_hard_stop_step"] is None else int(hparams["convdec_hard_stop_step"]),
            "optimizer": "AdamW",
            "target_normalization": "observed_training_latitude_weighted_per_field",
            "loss": "latitude_weighted_masked_mse_divided_by_weighted_valid_mass",
            "seed_grid": [4, 8],
            "stage_channels": ["C", "C/2", "C/4"],
            "equatorial_symmetry": True,
            "preset": args.subset if {k: getattr(args, k) for k in CONVDEC_ARG_DEFAULTS} == CONVDEC_ARG_DEFAULTS and args.subset in CONVDEC_PRESETS else None,
        }
    elif args.method == "sfno":
        ...
        cfg["sfno"] = {
            ...
            "hard_stop_step": None if hparams["sfno_hard_stop_step"] is None else int(hparams["sfno_hard_stop_step"]),
            "optimizer": "AdamW",
            "target_normalization": "observed_training_latitude_weighted_per_field",
            "loss": "latitude_weighted_masked_mse_divided_by_weighted_valid_mass",
            "grid": "legendre-gauss",
            ...
            "equatorial_symmetry": True,
            "preset": args.subset if {k: getattr(args, k) for k in SFNO_ARG_DEFAULTS} == SFNO_ARG_DEFAULTS and args.subset in SFNO_PRESETS else None,
        }
    ...
    if bool(getattr(args, "target_detrend", False)):
        cfg["target_detrend"] = TARGET_DETREND_CFG
    return cfg
