...
L_MAX = 21
_FORWARD_CACHE: dict[str, tuple[np.ndarray, np.ndarray]] = {}


def degree_starts(l_max: int = L_MAX) -> np.ndarray:
    """Start index of each spherical-harmonic degree block in the coefficient vector."""
    sizes = np.asarray([2 * l + 1 for l in range(l_max + 1)], dtype=np.int64)
    return np.concatenate([np.zeros(1, dtype=np.int64), np.cumsum(sizes)[:-1]])


def forward_sht_operator() -> tuple[np.ndarray, np.ndarray]:
    """Analysis operator ``F`` and per-coefficient scales ``sqrt_d``.

    ``F`` maps a flattened grid field to its T21 coefficients (the left inverse
    of the synthesis matrix under area-weighted quadrature). Rescaling the
    coefficients by ``sqrt_d`` makes their squared sum equal the area-weighted
    mean square of the band-limited field, so Parseval holds in the same metric
    as the area-weighted RMSE.
    """
    if "op" not in _FORWARD_CACHE:
        synthesis = load_inverse_sht_matrix().astype(np.float64)
        lat_w = load_latitude_weights().astype(np.float64)
        lat_w = lat_w / lat_w.sum()
        cell_w = np.repeat(lat_w / GRID_SHAPE[1], GRID_SHAPE[1])
        gram = synthesis.T @ (cell_w[:, None] * synthesis)
        d = np.diag(gram).copy()
        off = gram - np.diag(d)
        if np.abs(off).max() > 1.0e-6 * d.max():
            raise RuntimeError("Synthesis matrix is not orthogonal under quadrature weights.")
        analysis = (synthesis.T * cell_w[None, :]) / d[:, None]
        _FORWARD_CACHE["op"] = (analysis.astype(np.float32), np.sqrt(d).astype(np.float32))
    return _FORWARD_CACHE["op"]


def to_spectral(grid: np.ndarray) -> np.ndarray:
    """Analyse gridded fields into T21 spherical-harmonic coefficients."""
    grid = np.asarray(grid, dtype=np.float32)
    if grid.shape[-2:] != GRID_SHAPE:
        raise ValueError(f"Expected trailing grid shape {GRID_SHAPE}, got {grid.shape[-2:]}.")
    analysis, _ = forward_sht_operator()
    return grid.reshape(*grid.shape[:-2], GRID_SHAPE[0] * GRID_SHAPE[1]) @ analysis.T
