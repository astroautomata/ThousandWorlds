...
def is_in_target_physical_domain(values: Mapping[str, float]) -> bool:
    return (
        all(lower <= values[name] <= upper for name, (lower, upper) in TARGET_PHYSICAL_DOMAIN.items())
        and values["CO2"] + values["CH4"] <= 1.0
    )
